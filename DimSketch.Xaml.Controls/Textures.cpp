...
DirectX::RenderTargetTexture2D::RenderTargetTexture2D(ID3D11RenderTargetView * pRenderTargetView)
{
	m_pRenderTargetView = pRenderTargetView;
...
DepthStencilBuffer::DepthStencilBuffer(ID3D11Device* pDevice, unsigned int Width, unsigned int Height, _In_opt_ DXGI_FORMAT Format, UINT MuiltiSampleCount, UINT MuiltiSampleQuality)
	: Texture2D(pDevice, Width, Height, 1, Format, D3D11_USAGE_DEFAULT, D3D11_BIND_DEPTH_STENCIL, 0, 0, MuiltiSampleCount, MuiltiSampleQuality)
{
...
void DirectX::CubeTexture::Initialize(ID3D11Device * pDevice, const std::wstring(&TextureFiles)[6])
{
	for (int i = 0; i < 6; i++)
	{
		DirectX::ThrowIfFailed(CreateDDSTextureFromFile(pDevice, TextureFiles[i].c_str(), &(m_pTextures[i]), &(m_pTextureView[i])));
	}
}
