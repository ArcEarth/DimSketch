...
namespace DirectX{
	...
	void PrimitveDrawer::Initialize(ID3D11DeviceContext * pContext, bool rhs)
	{
		m_pContext = pContext;
		...
		m_pCylinder = DirectX::GeometricPrimitive::CreateCylinder(pContext,1.0f,2.0f,32U, rhs);
		m_pSphere = DirectX::GeometricPrimitive::CreateGeoSphere(pContext,1,3U, rhs);
		m_pCube = DirectX::GeometricPrimitive::CreateCube(pContext,2.0f, rhs);
		m_pCone = DirectX::GeometricPrimitive::CreateCone(pContext,1,1,32, rhs);
	...
	PrimitveDrawer::PrimitveDrawer(ID3D11DeviceContext * pContext, bool rhs)
	{
		Initialize(pContext, rhs);
	}
	...
	void XM_CALLCONV PrimitveDrawer::DrawTriangle(FXMVECTOR P0, FXMVECTOR P1, FXMVECTOR P2, GXMVECTOR Color)
	{
		...
		if (m_rhs)
			...
			m_pDirectXBatch->DrawTriangle(Vertices[2], Vertices[1], Vertices[1]);
	}
	...
	void XM_CALLCONV PrimitveDrawer::DrawQuad(FXMVECTOR P0, FXMVECTOR P1, FXMVECTOR P2, GXMVECTOR P3, CXMVECTOR Color)
	{
		...
		if (m_rhs)
			...
			m_pDirectXBatch->DrawQuad(Vertices[3], Vertices[2], Vertices[1], Vertices[0]);
	}
